...
describe.only('valueFromNestedObj', () => {
  ...
  describe('pathSeparator parameter', () => {
    it.each([
      ...
      'gives a useful error message if the pathSeparator parameter is not a string or null, but type $parameterType',
    ...
    it('works with a null pathSeparator, since when string.split() is given a null delimiter, it simply returns the whole path in an array', () => {
      // My inverse package, pathToNestedObj works fine with a null pathSeparator - so I want this to function the same way!
      // The only thing stopping it from working here was the parameter type checking I put in earlier - now fixed to allow null.
      const output = valueFromNestedObj('my.flat.path', null, {
        'my.flat.path': 'my nested value',
      });
      expect(output).toStrictEqual('my nested value');
    });
  });
